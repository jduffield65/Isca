...
def get_heat_cap_lambda_eff_approx(temp_surf: Union[float, np.ndarray, xr.DataArray],
                                   temp_atm: Union[float, np.ndarray, xr.DataArray],
                                   temp_rad_surf: Union[float, np.ndarray, xr.DataArray],
                                   temp_rad_atm: Union[float, np.ndarray, xr.DataArray],
                                   rh_atm: Union[float, np.ndarray, xr.DataArray],
...
Union[float, np.ndarray, xr.DataArray], Union[float, np.ndarray, xr.DataArray]]:
    ...
    gamma = omega * heat_cap_atmos * temp_col_amp_param / (4 * Stefan_Boltzmann * temp_surf ** 3)
    emiss_rad_surf = emiss_factor * (temp_rad_surf / temp_surf) ** 3 * (1 - temp_rad_surf_coef_amp)
    emiss_rad_atm = emiss_factor * (temp_rad_atm / temp_surf) ** 3 * (1 - temp_rad_atm_coef_amp)
    emiss_a3 = emiss_rad_surf * temp_col_coef_phase + emiss_rad_atm * (temp_col_coef_phase - temp_rad_atm_coef_phase)

    # # More Complete decomp
    # heat_cap_scaling = 1 + 2 * emiss_a3 * emiss_rad_atm * temp_rad_atm_coef_phase / (
    #             emiss_rad_atm + emiss_rad_surf) ** 2
    # heat_cap_scaling -= 2 * emiss_a3 * gamma / (emiss_rad_atm + emiss_rad_surf) ** 2
    # heat_cap_scaling -= emiss_rad_atm * temp_rad_atm_coef_phase / gamma
    # heat_cap_scaling *= emiss_factor * emiss_rad_surf / (emiss_rad_atm + emiss_rad_surf) ** 2 * \
    #                     heat_cap_atmos * temp_col_amp_param / heat_cap_surf
    # heat_cap_scaling += 1

    # Simpler decomp
    x = emiss_rad_surf + emiss_rad_atm
    heat_cap_scaling = 1
    heat_cap_scaling -= 2*temp_col_coef_phase*gamma/x
    heat_cap_scaling -= emiss_rad_atm*temp_rad_atm_coef_phase/gamma
    heat_cap_scaling *= emiss_factor * emiss_rad_surf / x ** 2 * \
                        heat_cap_atmos * temp_col_amp_param / heat_cap_surf
    heat_cap_scaling += 1



    lambda_s1_scaling0 = 1 - emiss_rad_surf*(x+gamma*temp_col_coef_phase)/(gamma**2+x**2 + 2*gamma*emiss_a3)*emiss_factor
    var = emiss_rad_surf / x ** 2 * (x + gamma * temp_col_coef_phase - (2 * gamma * emiss_a3 + gamma ** 2) / x)
    lambda_s1_scaling = 1 - var*emiss_factor

    var = emiss_rad_surf/x * (1 + ((temp_col_coef_phase*x-2*emiss_a3)*gamma-gamma**2)/x**2)
    var = emiss_rad_surf / x * (1 - ((temp_col_coef_phase * emiss_rad_surf) * gamma + gamma ** 2) / x ** 2)
    lambda_s1_scaling = 1 - var * emiss_factor

    # lambda_s1_scaling = 1 - emiss_factor * emiss_rad_surf / (emiss_rad_surf + emiss_rad_atm)
    # lambda_s1_scaling += emiss_factor*emiss_rad_surf*temp_col_coef_phase*gamma/(emiss_rad_atm+emiss_rad_surf)**2
    ...
    lambda_s1_scaling2 = 1 - lambda_s2 * lambda_a1 / lambda_s1 / lambda_a2_mod
    heat_cap_scaling2 = 1 + lambda_a1 * lambda_s2 / lambda_a2_mod ** 2 * heat_cap_atmos_mod / heat_cap_surf
    return lambda_s1, lambda_s1_scaling, heat_cap_scaling
