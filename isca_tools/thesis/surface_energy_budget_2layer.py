...
def get_feedback_params(temp_surf: Union[float, np.ndarray, xr.DataArray],
                        ...
                        temp_rad_surf_coef: Union[float, np.ndarray, xr.DataArray] = 0,
                        temp_rad_atm_coef: Union[float, np.ndarray, xr.DataArray] = 0
...
Union[float, np.ndarray, xr.DataArray], Union[float, np.ndarray, xr.DataArray]]:
    ...
    lambda_a1 = -(gamma['sh']['temp_atm'] + gamma['lh']['temp_atm'] +
                  gamma['lw_surf']['temp_rad_surf'] * (1-temp_rad_surf_coef))
    lambda_a2 = -(gamma['sh']['temp_atm'] + gamma['lh']['temp_atm'] +
                  gamma['lw_atm']['temp_rad_surf'] * (1-temp_rad_surf_coef))
    lambda_a3 = -gamma['lw_atm']['temp_rad_atm'] * (1-temp_rad_atm_coef)
    return lambda_s1, lambda_s2, lambda_a1, lambda_a2, lambda_a3
...
def get_heat_cap_lambda_eff(temp_surf: Union[float, np.ndarray, xr.DataArray],
                            ...
                            temp_diseqb_col_sphum: Union[float, np.ndarray, xr.DataArray],
                            p_col_sphum: float,
                            ...
                            temp_diseqb_atm_coef: Union[float, np.ndarray, xr.DataArray] = 0,
                            temp_diseqb_col_coef_amp: Union[float, np.ndarray, xr.DataArray] = 0,
                            temp_diseqb_col_coef_phase: Union[float, np.ndarray, xr.DataArray] = 0,
                            temp_diseqb_col_sphum_coef: Union[float, np.ndarray, xr.DataArray] = 0,
                            n_year_days: int = 360,
                            day_seconds: int = 86400,
                            assume_small_temp_diseqb_col_coef_phase: bool = True,
...
Union[float, np.ndarray, xr.DataArray], Union[float, np.ndarray, xr.DataArray]]:
    ...
    temp_col_sphum = temp_atm - temp_diseqb_col_sphum
    alpha_col_sphum = clausius_clapeyron_factor(temp_col_sphum, p_col_sphum)
    q_sat_col_sphum = sphum_sat(temp_col_sphum, p_col_sphum)
    mu = L_v/c_p * alpha_col_sphum * rh_col * q_sat_col_sphum * (1-temp_diseqb_col_sphum_coef)
    f = 1/(n_year_days*day_seconds)
    omega = 2 * np.pi * f

    if assume_small_temp_diseqb_col_coef_phase:
        # Taylor expansion of cos and sin to 1st order
        temp_diseqb_col_amp_param = 1 - temp_diseqb_col_coef_amp
        temp_diseqb_col_phase_param = (1 - temp_diseqb_col_coef_amp) * temp_diseqb_col_coef_phase
    else:
        temp_diseqb_col_amp_param = (1-temp_diseqb_col_coef_amp)*np.cos(temp_diseqb_col_coef_phase)
        temp_diseqb_col_phase_param = (1 - temp_diseqb_col_coef_amp) * np.sin(temp_diseqb_col_coef_phase)

    lambda_a2_mod = lambda_a2-omega*temp_diseqb_col_phase_param*heat_cap_atmos
    heat_cap_atmos_mod = heat_cap_atmos*(temp_diseqb_col_amp_param+mu)
