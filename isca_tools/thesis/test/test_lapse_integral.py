...
import os
import numpy as np
from ..lapse_integral import integral_lapse_dlnp_hydrostatic, fitting_2_layer_xr
...
class TestFitting(unittest.TestCase):
    ...
    def get_ds(self):
        ...
        with warnings.catch_warnings():
            ...
            ds['Z_ft'] = interp_hybrid_to_pressure(ds.Z3, ds.PS, ds.hyam, ds.hybm, ds.P0, np.atleast_1d(self.p_ft),
                                                   lev_dim='lev')
            ds['Z_ft'] = ds['Z_ft'].isel(plev=0, drop=True).load()
        return ds

    def test_const_lapse(self):
        ds = self.get_ds()
        var = fitting_2_layer_xr(ds.T, ds.P, ds.TREFHT, ds.PREFHT, ds.T_lcl_env, ds.p_lcl, ds.T_ft,
        ...
        var2 = fitting_2_layer_xr(ds.T, ds.P, ds.TREFHT, ds.PREFHT, ds.T_lcl_env, ds.p_lcl, ds.T_ft,
    ...
    def test_mod_parcel_lapse(self):
        ds = self.get_ds()
        var = fitting_2_layer_xr(ds.T, ds.P, ds.TREFHT, ds.PREFHT, ds.T_lcl_env, ds.p_lcl, ds.T_ft,
                                 self.p_ft, n_lev_above_upper2_integral=self.n_lev_above_integral,
                                 method_layer2='mod_parcel')
        mse_REFHT = moist_static_energy(ds.TREFHT, ds.QREFHT, ds.ZREFHT)
        mse_ft_sat = moist_static_energy(ds.T_ft, sphum_sat(ds.T_ft, self.p_ft), ds.Z_ft)
        epsilon = mse_REFHT - mse_ft_sat
        # Sanity check that lapse rate above LCL has negative modification for days with negative epsilon
        # This does necessarily have to be true, but is for test data
        self.assertTrue(epsilon.where(var[0].isel(layer=1) < 0).max() < 0)

        # Confirm that without `n_lev_above_upper2_integral`, bulk lapse and below LCL fitting is the same, but above is different
        var2 = fitting_2_layer_xr(ds.T, ds.P, ds.TREFHT, ds.PREFHT, ds.T_lcl_env, ds.p_lcl, ds.T_ft,
                                  self.p_ft, n_lev_above_upper2_integral=0, method_layer2='mod_parcel')
        self.assertEqual(list(var[0].values.flatten()), list(var2[0].values.flatten()))
        for i in range(1, 3):
            self.assertEqual(list(var[i].isel(layer=0).values.flatten()), list(var2[i].isel(layer=0).values.flatten()))
            self.assertNotEqual(list(var[i].isel(layer=1).values.flatten()), list(var2[i].isel(layer=1).values.flatten()))
    ...
    def test_vary_lcl_fit(self):
        # Test constant lapse fitting but considering multiple LCL pressures for each location. Then chose best one
        for sf in [1, 100]:
            ds = self.get_ds()
            # Check with ludicrous lcl pressure, see if can ignore
            p_lcl_use = [(10 ** (np.log10(ds.p_lcl) + sf * self.lcl_mod[i])) for i in range(self.n_lcl_mod)]
            ds['p_lcl'] = xr.concat(p_lcl_use, dim=xr.DataArray(sf * self.lcl_mod, name='lcl_mod', dims='lcl_mod'))
            ds['T_lcl_env'] = get_var_at_plev(ds.T, ds.P, ds.p_lcl)
            for key in ['const', 'mod_parcel']:
                var = fitting_2_layer_xr(ds.T, ds.P, ds.TREFHT, ds.PREFHT, ds.T_lcl_env, ds.p_lcl, ds.T_ft,
                                         self.p_ft, n_lev_above_upper2_integral=self.n_lev_above_integral,
                                         method_layer2=key)
                var_error = var[2].sum(dim='layer', skipna=False)
                ind_best = var_error.argmin(dim='lcl_mod', skipna=True)
                if sf == 100:
                    # Ensure assigned best index to where lcl_mod = 0 as others are out of bounds
                    self.assertTrue(np.abs(self.lcl_mod[ind_best]).max() < self.tol)
                var_integral = var[1].sum(dim='layer', skipna=False)
