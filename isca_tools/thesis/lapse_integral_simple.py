...
def fitting_2_layer_xr(temp_env_lev: xr.DataArray,
    ...
    func_kwargs = {'method_layer1': method_layer1,
                   ...
                   'temp_surf_lcl_calc': temp_surf_lcl_calc, 'const_layer1_method': const_layer1_method,
                   'mod_parcel_method': mod_parcel_method,
                   'sanity_check': sanity_check, 'p_range_calc': p_range_calc, 'force_parcel': force_parcel}
...
def get_temp_2_layer_approx(p_lev: np.ndarray, temp_env_surf: float, p_surf: float, rh_surf: float,
    ...
    if method_layer2 == 'const':
        ...
        temp_lev = np.where(p_lev < p_upper,
                            np.asarray([get_temp_mod_parcel(rh_surf, p_surf, p_lev[i], lapse_layer1 - lapse_dry,
                                                            lapse_layer2, temp_env_surf, method=mod_parcel_method)
                                        for i in range(p_lev.size)]), temp_lev)
    else:
        raise ValueError(f'method_layer2 = {method_layer2} not recognized.')
    return temp_lev
