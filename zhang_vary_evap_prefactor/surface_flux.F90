...
subroutine surface_flux_1d (                                           &
     ...
     u_surf,    v_surf,                                                &
     land_evap_prefactor_2d,                                           & !YZ 20240726
...
!        flux_q    =  rho_drag * land_evap_prefactor * (land_humidity_prefactor*q_surf0 - q_atm) ! flux of water vapor  (Kg/(m**2 s))
        flux_q    =  rho_drag * land_evap_prefactor_2d * (land_humidity_prefactor*q_surf0 - q_atm) ! YZ 20240726flux of water vapor  (Kg/(m**2 s))
        dedq_surf = 0
!        dedt_surf =  rho_drag * land_evap_prefactor * (land_humidity_prefactor*q_sat1 - q_sat) *del_temp_inv
        dedt_surf =  rho_drag * land_evap_prefactor_2d * (land_humidity_prefactor*q_sat1 - q_sat) *del_temp_inv ! YZ 20240726
!        dedq_surf = rho_drag 
!        dedt_surf = 0
     elsewhere
        flux_q    =  rho_drag * (q_surf0 - q_atm) ! flux of water vapor  (Kg/(m**2 s))
...
subroutine surface_flux_0d (                                                 &
  ...
  avail(1)       = avail_0
  land_evap_prefactor_0d(1) = 1.0!land_evap_prefactor YZ 20240726
  ...
  call surface_flux_1d (                                                 &
       ...
       depth_change_lh_1d, depth_change_conv_1d, depth_change_cond_1d,   &
       u_surf,    v_surf,                                                &
       land_evap_prefactor_0d,  & !YZ 20240726
       rough_mom, rough_heat, rough_moist, rough_scale, gust,            &
       flux_t, flux_q, flux_r, flux_u, flux_v,                           &
       cd_m,      cd_t,       cd_q,                                      &
       w_atm,     u_star,     b_star,     q_star,                        &
...
subroutine surface_flux_2d (                                           &
  ...
  do j = 1, size(t_atm,2)
     call surface_flux_1d (                                           &
          ...
          u_surf(:,j),    v_surf(:,j),                                                                    &
          land_evap_prefactor_2d(:,j),                                                                    & !YZ 20240726
